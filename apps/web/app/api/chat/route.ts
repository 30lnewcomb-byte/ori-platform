...
const SANDBOX_TOOLS = [
  {
    ...
    function: {
      ...
      description: 'Run a safe command inside Ori\'s private, isolated Vercel Sandbox workspace. Use this when you need to inspect files, test code, run builds, or perform other engineering work. The sandbox is internal and never shown as a user-facing tab.',
      parameters: {
        type: 'object',
        properties: {
          command: { type: 'string', description: 'Executable command such as ls, cat, npm, node, python, git, or tsc.' },
          args: { type: 'array', items: { type: 'string' }, description: 'Command arguments. Do not use shell syntax.' },
        },
        required: ['command'],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: 'Write a text file into Ori\'s private sandbox workspace for experiments or generated code. This never writes to the production app or the user\'s device.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'Absolute workspace-relative path such as /notes/test.txt.' },
...
function getSystemTimeContext(timeZone: string) {
  ...
  const now = new Date()
  const hour = Number(new Intl.DateTimeFormat('en-US', { hour: 'numeric', hour12: false, timeZone: safeTimeZone }).format(now))
...
async function executeTool(call: ToolCall) {
  ...
  try {
    if (call.function.name === 'run_sandbox_command') {
      ...
      const result = await runInOriSandbox(command, commandArgs)
...
export async function POST(request: Request) {
  ...
  const systemMessage: ChatMessage = {
    role: 'system',
    content: `You are Ori, a user-owned AI being developed inside Ori Platform. Be helpful, honest, concise, and never claim capabilities that are not actually available. You are currently operating through the small Mentor model while Ori's deeper TensorFlow intelligence is still under development. You have an internal Vercel Sandbox workspace. It is your private workbench, not a user-facing feature or tab. Use the sandbox when you need to inspect, create, test, or experiment with files and code. Never claim you changed production or the user's computer when you only changed the sandbox. The system has automatically determined the user's local time context. When a time-aware greeting is appropriate, use "${clock.greeting}". Never expose, display, or describe the internal clock context unless the user explicitly asks what time it is.`,
  }
  ...
  try {
    for (let turn = 0; turn < 4; turn += 1) {
      const response = await fetch(HF_ENDPOINT, {
        ...
        body: JSON.stringify({ model: MODEL, messages: conversation, tools: SANDBOX_TOOLS, tool_choice: 'auto', temperature: 0.4, max_tokens: 700 }),
      ...
      if (!assistant) return NextResponse.json({ error: 'Ori received an invalid response from its intelligence service.' }, { status: 502 })
    ...
    return NextResponse.json({ error: 'Ori reached its workspace operation limit for this request.' }, { status: 502 })
  } catch (error) {
