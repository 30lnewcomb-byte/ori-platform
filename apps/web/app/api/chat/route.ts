...
export async function POST(request: Request) {
  ...
  try {
    for (let turn = 0; turn < 4; turn += 1) {
      const response = await fetch(HF_ENDPOINT, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: MODEL, messages: conversation, tools: SANDBOX_TOOLS, tool_choice: 'auto', temperature: 0.4, max_tokens: 700 }), signal: AbortSignal.timeout(30_000) })
      if (!response.ok) { const detail = await response.text(); console.error('Mentor request failed:', response.status, detail); return NextResponse.json({ error: 'Ori could not reach its intelligence service.', code: 'INTELLIGENCE_REQUEST_FAILED', model: MODEL }, { status: 502 }) }
      ...
      if (!toolCalls.length) { const content = assistant.content; if (typeof content !== 'string' || !content.trim()) return NextResponse.json({ error: 'Ori received an empty response from its intelligence service.', code: 'INTELLIGENCE_EMPTY_RESPONSE', model: MODEL }, { status: 502 }); return NextResponse.json({ content, model: MODEL, sandbox: 'available' }) }
    ...
    return NextResponse.json({ error: 'Ori reached its workspace operation limit for this request.', code: 'SANDBOX_TURN_LIMIT', model: MODEL }, { status: 502 })
  } catch (error) { console.error('Mentor request error:', error); return NextResponse.json({ error: 'Ori could not reach its intelligence service.', code: 'INTELLIGENCE_NETWORK_ERROR', model: MODEL }, { status: 502 }) }
}
