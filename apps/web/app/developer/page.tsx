  ...
  return (
    <main className={styles.page}>
      <section className={styles.hero}>
        <div>
          ...
          <p className={styles.lede}>
            A developer platform for connecting applications to Ori&apos;s intelligence,
            tools, projects, and future execution capabilities.
          </p>
          <div className={styles.actions}>
            <a className={styles.primary} href="#capabilities">Explore the platform</a>
        ...
        <div className={styles.statusCard} aria-label="Developer platform status">
          ...
          <p>The public surface is being built from stable platform boundaries rather than exposing internal implementation details.</p>
        </div>
      </section>
        ...
        <div className={styles.grid}>
          {capabilities.map((item) => (
            <article className={styles.card} key={item.title}>
              ...
              <h3>{item.title}</h3>
              <p>{item.text}</p>
              <span className={styles.coming}>Foundation in development</span>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className={styles.platform} aria-labelledby="platform-heading">
        <div>
          <p className={styles.eyebrow}>PLATFORM SURFACE</p>
          <h2 id="platform-heading">One platform. Clear boundaries.</h2>
          <p>
            Ori Platform is the main product. The Developer Platform sits inside it and gives developers stable interfaces around projects, intelligence, tools, authentication, events, and documentation.
          </p>
        </div>
        <div className={styles.pieceGrid} aria-label="Developer platform components">
          {platformPieces.map((piece, index) => (
            <span key={piece} className={index === 0 ? styles.pieceFeatured : styles.piece}>{piece}</span>
          ))}
        </div>
      </section>
      ...
      <section className={styles.boundary} aria-labelledby="boundary-heading">
        <div>
          ...
          <h2 id="boundary-heading">The platform is not the runtime.</h2>
        </div>
        <div className={styles.flow} aria-label="Platform architecture">
          <span>Your application</span>
          <b>↓</b>
          <span>Ori Developer Platform API</span>
          <b>↓</b>
          <span>Ori intelligence · tools · platform services</span>
        </div>
      </section>
